...
8. **Check file overlaps between tasks:**
   ```bash
   mac10 check-overlaps <request_id>
   ```
...
**Rule 4: Respect coupling boundaries** — coupled files in SAME task. After creating all tasks, run `mac10 check-overlaps` and serialize CRITICAL overlaps with `depends_on`
